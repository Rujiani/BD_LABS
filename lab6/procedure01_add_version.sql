...
BEGIN
    SELECT document_id
    INTO v_document_id
    FROM ws_document_version
    WHERE version_id = p_previous_version_id;
    ...
    VALUES (
        ...
        v_latest.informant_id,
        v_latest.body_preview
    )
    ...
    IF p_theme_id IS NOT NULL THEN
        ...
        VALUES (v_new_version_id, p_theme_id);
    ELSE
        INSERT INTO ws_version_theme (version_id, theme_id)
        SELECT v_new_version_id, vt.theme_id
        FROM ws_version_theme vt
        WHERE vt.version_id = v_latest.version_id;
    END IF;

    RAISE NOTICE 'Создана версия % документа % (version_no=%)',
        v_new_version_id, v_document_id, v_new_no;
END;
$$;
